...
class ClusterPooling(torch.nn.Module):
    ...
    def _merge_edges(
        ...
        if len(edge_score) > 0:
            max_score = edge_score.max()
            min_score = edge_score.min()
            threshold = min_score + (max_score - min_score) * 0.99
        else:
            threshold = 0
        ...
        S = to_dense_adj(edge_contract, edge_attr=edge_score,
        ...
        edge_index_out = torch.cat([edge_index_out,torch.cat([(C.sum(0)>1).nonzero(),(C.sum(0)>1).nonzero()],dim=1).t()],dim=1)
